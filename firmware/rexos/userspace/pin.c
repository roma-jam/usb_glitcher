...
{
    ipc_post_exo(HAL_CMD(HAL_PIN, STM32_GPIO_ENABLE_EXTI), pin, flags, 0);
}

void pin_disable_exti(unsigned int pin)
{
    ipc_post_exo(HAL_CMD(HAL_PIN, STM32_GPIO_DISABLE_EXTI), pin, 0, 0);
}
#else
...
void pin_enable_exti(unsigned int pin, unsigned int flags)
{
    ack(object_get(SYS_OBJ_CORE), HAL_CMD(HAL_PIN, STM32_GPIO_ENABLE_EXTI), pin, flags, 0);
}

void pin_disable_exti(unsigned int pin)
{
    ack(object_get(SYS_OBJ_CORE), HAL_CMD(HAL_PIN, STM32_GPIO_DISABLE_EXTI), pin, 0, 0);
}
#endif //EXODRIVERS
